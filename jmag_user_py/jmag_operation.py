...
def run_case(study,case): 
    app,model,mstudy,result = initialize_jmag()
    mstudy.DeleteResult()
    if isinstance(case, int):
        if app.GetStudy(study).GetReport().HasWarningMessage():
            app.GetStudy(study).ApplyAllCasesCadParameters()
        mstudy.DeleteResult()
        app.GetModel(0).GetStudy(study).Run()
    if isinstance(case, list):
        for casei in case:
            app.SetCurrentStudy(study)
			#app.GetCurrentStudy().SetCurrentCase(case - 1)
            if app.GetStudy(study).GetReport().HasWarningMessage():
                app.GetStudy(study).ApplyAllCasesCadParameters()
            mstudy.DeleteResult()
        app.GetModel(0).GetStudy(study).RunAllCases()
...
def get_data(objname,study,case):
    ...
    if isinstance(case, int):
        ...
        para_i=-1
        for i in range(mstudy.GetDesignTable().NumParameters()):
            if "Equation" in mstudy.GetDesignTable().ParameterName(i) and 'Equation parameters: '+objname in mstudy.GetDesignTable().ParameterName(i):
                para_i=i
        if para_i>=0:
			#if app.GetCurrentStudy().GetCurrentCase()+1 !=case:
				#app.View().SetCurrentCase(case)
            re=app.GetCurrentStudy().GetDesignTable().GetEquation(objname).GetValue(case - 1)
        else:
            re = mstudy.GetResponseData(objname, case - 1)
            if len(re)>0:
                re=re[0]
		##ttt=time.time_ns()-ttt
		#print('get_data:',objname,'=',re,'  Time:',ttt/1e9,'s')
        return re
    if isinstance(case, list):
        re_all = 0
        for casei in case:
            ...
            app.SetCurrentStudy(study)
            mstudy=app.GetCurrentStudy()
			#app.GetCurrentStudy().SetCurrentCase(casei - 1)
			#app.GetCurrentStudy().ApplyCadParameters()
            para_i=-1
            for i in range(mstudy.GetDesignTable().NumParameters()):
                if "Equation" in mstudy.GetDesignTable().ParameterName(i) and 'Equation parameters: '+objname in mstudy.GetDesignTable().ParameterName(i):
                    para_i=i
            if para_i>=0:
				#if app.GetCurrentStudy().GetCurrentCase()+1 !=casei:
					#app.View().SetCurrentCase(casei)
                    re=app.GetCurrentStudy().GetDesignTable().GetEquation(objname).GetValue(casei - 1)
            else:
                re = mstudy.GetResponseData(objname, casei - 1)
                if len(re)>0:
                    re=re[0]
            re_all += re 
			##ttt=time.time_ns()-ttt
...
def get_datas(objname,study,case): ###here objnames is a list
	#ttt=time.time_ns()
	try:
		len(objname)
	except TypeError:
		objname=[objname]
	datas=[get_data(iname,study,case) for iname in objname]
	
	return datas

def set_para(varname,varvalue,study,case):
    app,model,mstudy,result = initialize_jmag()
    if isinstance(case, int):
	#ttt=time.time_ns()
        app.SetCurrentStudy(study)
		#app.GetCurrentStudy().SetCurrentCase(case - 1)
        if get_data(varname, study, case) != varvalue:
            app.GetStudy(study).DeleteResult()
            for i in range(app.GetStudy(study).GetDesignTable().NumParameters()):
                if app.GetStudy(study).GetDesignTable().ParameterTypeName(i) == "Equation" and 'Equation parameters: '+varname == app.GetCurrentStudy().GetDesignTable().ParameterName(i):
					#print(app.GetCurrentStudy().GetDesignTable().ParameterName(i))
                    var_num = i
            app.GetStudy(study).GetDesignTable().SetValue(case - 1, var_num, str(varvalue))
    if isinstance(case, list):
        for casei in case:
            app.SetCurrentStudy(study)
            if get_data(varname, study,casei) != varvalue:
                app.GetStudy(study).DeleteResult()
			#app.GetCurrentStudy().SetCurrentCase(casei - 1)
                for i in range(app.GetStudy(study).GetDesignTable().NumParameters()):
                    if app.GetStudy(study).GetDesignTable().ParameterTypeName(i) == "Equation" and 'Equation parameters: '+varname == app.GetCurrentStudy().GetDesignTable().ParameterName(i):
						#print(app.GetCurrentStudy().GetDesignTable().ParameterName(i))
                        var_num = i
                app.GetStudy(study).GetDesignTable().SetValue(casei - 1, var_num, str(varvalue))
    app.GetStudy(study).ApplyCadParameters()

def set_paras(varnames,varvalues,study,case):
	app,model,mstudy,result = initialize_jmag()
	...
	if isinstance(case, int):
		try:
			...
			varnames=[varnames]
			varvalues=[varvalues]
		exit=[get_data(iname,study,case) for iname in varnames]
		if exit !=varvalues:
			app.GetStudy(study).DeleteResult()
			for iname in varnames:
				for i in range(app.GetStudy(study).GetDesignTable().NumParameters()):
					if app.GetStudy(study).GetDesignTable().ParameterTypeName(i) == "Equation" and 'Equation parameters: '+iname == app.GetCurrentStudy().GetDesignTable().ParameterName(i):
						var_num = i
				app.GetStudy(study).GetDesignTable().SetValue(case - 1, var_num, str(varvalues[varnames.index(iname)]))
		if app.GetStudy(study).GetReport().HasWarningMessage():
			app.GetStudy(study).ApplyCadParameters()
	if isinstance(case, list):
		for casei in case:
			try:
				...
				varnames=[varnames]
				varvalues=[varvalues]
			exit=[get_data(iname,study,casei) for iname in varnames]
			if exit !=varvalues:
				app.GetStudy(study).DeleteResult()
				for iname in varnames:
					for i in range(app.GetStudy(study).GetDesignTable().NumParameters()):
						if app.GetStudy(study).GetDesignTable().ParameterTypeName(i) == "Equation" and 'Equation parameters: '+iname == app.GetCurrentStudy().GetDesignTable().ParameterName(i):
							var_num = i
					app.GetStudy(study).GetDesignTable().SetValue(casei - 1, var_num, str(varvalues[varnames.index(iname)]))
			if app.GetStudy(study).GetReport().HasWarningMessage():
				app.GetStudy(study).ApplyCadParameters()        
...
def get_datas_set_paras(objname,varname,varvalue,study,case):
	...
	if isinstance(case, int):
	#app.GetCurrentStudy().SetCurrentCase(case - 1)
		if [get_data(iname, study, case) for iname in varname] !=varvalue:
			app.GetStudy(study).DeleteResult()
			set_paras(varname, varvalue, study, case)
			app.GetCurrentStudy().Run()
		if app.GetStudy(study).GetReport().HasWarningMessage():
			app.GetStudy(study).ApplyCadParameters()
		if app.GetStudy(study).HasResult()==False:app.GetStudy(study).RunAllCases()
		#app.GetCurrentStudy().ApplyCadParameters()
	if isinstance(case, list):
		if [get_data(iname, study, case[0]) for iname in varname] !=varvalue:
				#app.GetCurrentStudy().SetCurrentCase(casei - 1)
				app.GetCurrentStudy().DeleteResult()
				set_paras(varname, varvalue, study, case)
				app.GetCurrentStudy().RunAllCases()
		if app.GetCurrentStudy().GetReport().HasWarningMessage():
			app.GetCurrentStudy().ApplyAllCasesCadParameters()
		if app.GetStudy(study).HasResult()==False:app.GetStudy(study).RunAllCases()
		#app.GetCurrentStudy().ApplyCadParameters()
